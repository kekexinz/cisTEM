...
void Project3DApp::DoInteractiveUserInput()
{
	...
	{	
		...
		psi_step = my_input->GetFloatFromUser("In-plane angular step (psi) for projection", "In-plane angular step size for grid projection", "0.0", 0.1);
		apply_CTF = my_input->GetYesNoFromUser("Apply CTF", "Should the CTF be applied to the output projections?", "No");
		apply_shifts = my_input->GetYesNoFromUser("Apply shifts", "Should the particle translations be applied to the output projections?", "No");
		if (apply_CTF == true)
		{
			...
			defocus_1 = my_input->GetFloatFromUser("defocus1", "defocus 1", "4000", 100, 50000);
			defocus_2 = my_input->GetFloatFromUser("defocus2", "defocus 2", "4000", 100, 50000);
			defocus_angle = my_input->GetFloatFromUser("defocus angle", "defocus angle", "0.0", 0.0, 500000);
			phase_shift = my_input->GetFloatFromUser("phase shift", "phase shift", "0.0", 0.0);
			...
			image_shift_x = my_input->GetFloatFromUser("Particle shift along x (A)", "Average particle shift along the x axis as a result of beam tilt in A", "0.0", -1.0, 1.0);
			image_shift_y = my_input->GetFloatFromUser("Particle shift along y (A)", "Average particle shift along the y axis as a result of beam tilt in A", "0.0", -1.0, 1.0);
			x_shift = my_input->GetFloatFromUser("Projection offcenter along x (A)", "Average offset along the x axis as a result of projection", "0.0", -5.0, 5.0);
			y_shift = my_input->GetFloatFromUser("Projection offcenter along y (A)", "Average offset along the y axis as a result of projection", "0.0", -5.0, 5.0);
		}
	}
...
//	my_current_job.Reset(14);
	my_current_job.ManualSetArguments("tttiifffftbbbbbfffffffffffffffi",	input_star_filename.ToUTF8().data(),
...
//	float    particle_shift_y					= my_current_job.arguments[12].ReturnFloatArgument();
	...
	float    psi_step							= my_current_job.arguments[16].ReturnFloatArgument();
	float    voltage_kV							= my_current_job.arguments[17].ReturnFloatArgument();
	float 	 spherical_aberration_mm			= my_current_job.arguments[18].ReturnFloatArgument();
	float    amplitude_contrast					= my_current_job.arguments[19].ReturnFloatArgument();
	float    defocus_1 							= my_current_job.arguments[20].ReturnFloatArgument();
	float    defocus_2 							= my_current_job.arguments[21].ReturnFloatArgument();
	float    defocus_angle 					    = my_current_job.arguments[22].ReturnFloatArgument();
	float    phase_shift 					    = my_current_job.arguments[23].ReturnFloatArgument();
	float    beam_tilt_x						= my_current_job.arguments[24].ReturnFloatArgument();
	float    beam_tilt_y						= my_current_job.arguments[25].ReturnFloatArgument();
	float    image_shift_x						= my_current_job.arguments[26].ReturnFloatArgument();
	float    image_shift_y						= my_current_job.arguments[27].ReturnFloatArgument();  
	float    x_shift							= my_current_job.arguments[28].ReturnFloatArgument();  
	float    y_shift							= my_current_job.arguments[29].ReturnFloatArgument();  
	...
	int total_number_of_projections;
	float mask_falloff = 10.0;
	wxArrayInt lines_to_process;
	float coordinates[3];
	long text_file_access_type;
	text_file_access_type = OPEN_TO_WRITE;
	NumericTextFile coordinate_file("coord.txt", text_file_access_type, 3);
	coordinate_file.WriteCommentLine("         Phi          Theta            Psi      ");
	cisTEMParameterLine input_parameters;

	cisTEMParameters input_star_file;
	if (project_based_on_star == true) input_star_file.ReadFromcisTEMStarFile(input_star_filename);
	...
	if (project_based_on_star == false)
	{
		...
		global_euler_search.InitGrid(my_symmetry, angular_step, 0.0f, 0.0f, 360, psi_step, 0.0, pixel_size / 8, parameter_map, 20);
		...
		global_euler_search.CalculateGridSearchPositions(false); // get # of phi/theta pairs

		number_of_rotations = 0;

		for (current_psi = 0.0; current_psi <= 360.0; current_psi += psi_step)
		{
			number_of_rotations++;		// get # of psi
		}

		total_number_of_projections=0;
		for (current_search_position = 0; current_search_position <= global_euler_search.number_of_search_positions; current_search_position++)
		{
			//loop over each rotation angle
			for (current_psi = 0.0; current_psi <= 360.0; current_psi += psi_step)
			{
				total_number_of_projections++;  // get # of total projections
			}
		}
	}
...
// Read whole parameter file to work out average image_sigma_noise and average score
	...
	if (project_based_on_star == true) number_of_projections_to_calculate = lines_to_process.GetCount();
	else number_of_projections_to_calculate = global_euler_search.number_of_search_positions;
	...
	#pragma omp parallel num_threads(max_threads) default(none) shared(global_random_number_generator, input_star_file, first_particle, last_particle, apply_CTF, apply_shifts, \
			pixel_size, output_file, add_noise, wanted_SNR, apply_mask, mask_radius, my_progress, lines_to_process, image_counter, projection_3d, input_file, global_euler_search, number_of_projections_to_calculate, project_based_on_star,\
			voltage_kV, spherical_aberration_mm, amplitude_contrast, defocus_1, defocus_2, defocus_angle, phase_shift, beam_tilt_x, beam_tilt_y, image_shift_x, image_shift_y, psi_step, x_shift, y_shift, coordinate_file) \
	private(current_image, current_psi, current_slice, input_parameters, my_parameters, my_ctf, projection_image, final_image, variance, coordinates)
	{
		...
		for (current_image = 0; current_image < number_of_projections_to_calculate; current_image++)
		{
			...
			{	
				for (current_psi = 0.0; current_psi <= 360.0; current_psi += psi_step)
				{
					my_parameters.Init(global_euler_search.list_of_search_parameters[current_image][0], global_euler_search.list_of_search_parameters[current_image][1], current_psi, x_shift, y_shift);
					coordinates[0] = global_euler_search.list_of_search_parameters[current_image][0];
					coordinates[1] = global_euler_search.list_of_search_parameters[current_image][1];
					coordinates[2] = current_psi;
					projection_3d.ExtractSlice(projection_image, my_parameters);
					my_ctf.Init(voltage_kV, spherical_aberration_mm, amplitude_contrast, defocus_1, defocus_2, defocus_angle, 0.0, 0.0, 0.0, pixel_size, phase_shift, -beam_tilt_x / 1000.0f, -beam_tilt_y / 1000.0f, -image_shift_x, -image_shift_y);
					projection_image.complex_values[0] = projection_3d.complex_values[0];

					if (apply_CTF) projection_image.ApplyCTF(my_ctf, false, true);
					if (apply_shifts) projection_image.PhaseShift(x_shift / pixel_size, y_shift / pixel_size);
					projection_image.SwapRealSpaceQuadrants();
					projection_image.BackwardFFT();
					...
					#pragma omp ordered
					final_image.WriteSlice(&output_file, current_slice+1);
