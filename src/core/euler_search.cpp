...
void EulerSearch::InitGrid(wxString wanted_symmetry_symbol, float wanted_angular_step_size, float wanted_phi_start, float wanted_phi_max, float wanted_theta_start, float wanted_theta_max, float wanted_psi_max, float wanted_psi_step, float wanted_psi_start, float wanted_resolution_limit, ParameterMap &wanted_parameter_map, int wanted_parameters_to_keep)
{
...
//			wxPrintf("i, phi, theta = %i, %f, %f\n", i, list_of_search_parameters[i][0], list_of_search_parameters[i][1]);
		...
		else projection_image->CopyFrom(&projections[i]);
