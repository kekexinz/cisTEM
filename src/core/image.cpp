...
void Image::Compute1DPowerSpectrumCurve(Curve* curve_with_average_power, Curve* curve_with_number_of_values, Image* mask, bool average_amplitudes_not_intensities) {
    ...
    MyDebugAssertTrue(curve_with_average_power->number_of_points > 0, "Curve not setup");
    MyDebugAssertTrue(curve_with_average_power->data_x[0] == 0.0, "Curve does not start at x = 0\n");
    MyDebugAssertTrue(curve_with_average_power->data_x[curve_with_average_power->number_of_points - 1] >= 0.49999, "Curve does not go to at least x = 0.5\n");
    MyDebugAssertTrue(curve_with_average_power->number_of_points == curve_with_number_of_values->number_of_points, "Curves need to have the same number of points");
    ...
    // We need to back-transform, mask, and re-transform.
...
void Image::DetectIceBackground(Image* background_mask, float particle_radius_angstroms, float pixel_size, float tile_size_multiplier) {
    ...
    std::vector<int>   tile_x_coords(total_tiles);
    std::vector<int>   tile_y_coords(total_tiles);
    ...
    for ( int ty = 0; ty < num_tiles_y; ty++ ) {
        for ( int tx = 0; tx < num_tiles_x; tx++ ) {
            ...
            for ( int jj = start_y; jj < start_y + tile_size && jj < logical_y_dimension; jj++ ) {
                for ( int ii = start_x; ii < start_x + tile_size && ii < logical_x_dimension; ii++ ) {
                    long  addr   = jj * (logical_x_dimension + padding_jump_value) + ii;
                    float val    = real_values[addr];
                    float val_bp = bandpass_image.real_values[addr];
                    sum += val;
                    sum_sq += val * val;
                    sum_bp += val_bp;
            ...
            tile_x_coords[tile_idx]  = tx;
            tile_y_coords[tile_idx]  = ty;
            tile_idx++;
        }
    }
    ...
    for ( int i = 0; i < total_tiles; i++ ) {
        int tx = tile_x_coords[i];
        int ty = tile_y_coords[i];
        ...
        if ( fano_ok && mean_ok ) {
            ...
            for ( int jj = start_y; jj < start_y + tile_size && jj < logical_y_dimension; jj++ ) {
                for ( int ii = start_x; ii < start_x + tile_size && ii < logical_x_dimension; ii++ ) {
                    long addr                        = jj * (logical_x_dimension + padding_jump_value) + ii;
                    background_mask->real_values[addr] = 1.0f;
                }
            }
            tiles_accepted++;
        }
    ...
    long total_pixels    = 0;
    for ( int jj = 0; jj < logical_y_dimension; jj++ ) {
        for ( int ii = 0; ii < logical_x_dimension; ii++ ) {
            long addr = jj * (logical_x_dimension + padding_jump_value) + ii;
            if ( background_mask->real_values[addr] > 0.5f ) {
                ice_pixel_count++;
            }
            total_pixels++;
        }
