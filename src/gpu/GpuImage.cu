...
__global__ void ComputeAmplitudeSpectrumFull2DKernel(cufftComplex* ref_complex_values,int4 dims, int3 physical_upper_bound_complex);
...
static __device__ cufftComplex CB_complexConjMulLoad_16f(void* dataIn, size_t offset, void* callerInfo, void* sharedPtr)
 {
  ...
  //return (cufftComplex)ScaleComplex(temp_cc, 100.0f);
 }
...
void GpuImage::ComputeAmplitudeSpectrumFull2D()
{
  ...
  dim3 gridDims = dim3((this->real_memory_allocated + threadsPerBlock.x - 1) / threadsPerBlock.x,1,1);
	pre_checkErrorsAndTimingWithSynchronization(cudaStreamPerThread);
	 ComputeAmplitudeSpectrumFull2DKernel<< <gridDims, threadsPerBlock,0,cudaStreamPerThread>> >(this->complex_values_gpu,this->dims,this->physical_upper_bound_complex);
  checkErrorsAndTimingWithSynchronization(cudaStreamPerThread);
...
__global__ void ComputeAmplitudeSpectrumFull2DKernel(cufftComplex* ref_complex_values,int4 dims, int3 physical_upper_bound_complex)
{
	...
	if (coords.x < dims.w / 2 && coords.y < dims.y && coords.z < dims.z)
        ...
        //s = 0.0f;
				...
				//ref_complex_values[address] = (cufftComplex)ComplexScale((Complex)ref_complex_values[address],0.0f);
